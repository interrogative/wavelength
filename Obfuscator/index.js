...
function visitChunk(chunk, opmap, settings = {}, top = false) {
    ...
    let fakeConstants = [ 
        'local', 'game', 'HttpGet', 'require', 'loadstring', 'assert', 'load', 'workspace',
        ...
        'whitelisted', 'if', 'inject', 'pairs', 'select', 'assert', 'pcall', 'typeof',
        'type', 'unpack', 'coroutine', 'table', 'rawget', 'rawset', 'new', '__instr__', '__const__', '__init__',
        'tostring', 'Chunk', 'sizeof', 'hi man whatcha doin here'
    ]
    let l = chunk.Const.length + 3
    consts.push('coems sec ontop fr')
    for (let i = 0; i < l; i++)
...
module.exports = {
    ModifyTree: (tree, settings) => {
        ...
        tree.Headers.Signature = 'coems Secure Copyright Boronide'
